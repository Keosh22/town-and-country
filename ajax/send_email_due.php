...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
  ...
  $query2 = "SELECT 
    ...
    WHERE collection_list.month = :current_month AND collection_list.email_status = :not_sent AND collection_list.status = :due AND collection_list.archive = :ACTIVE LIMIT 50
     ";
  $data2 = ["current_month" => $current_month, "not_sent" => $not_sent, "due" => $due, "ACTIVE" => $ACTIVE];
  ...
  if ($stmt2->rowCount() > 0) {
    while ($result2 = $stmt2->fetch()) {
      ...
      $email = $email_address;
      $subject = "Reminder Monthly Dues Overdue";
      $message = '<div class="container-fluid">
        <div class="row">
          ...
          <div class="row">
            ...
            <div class="col-12">
              <p>I hope you are doing well! This is a gentle reminder that the monthly dues payment for this month of <b>' . $collection_month . '</b> from your property: <b>' . $address . '</b> is now overdue.
              </p>
              <br>
              <p>Please ensure that this payment is made as soon as possible. If there are any issues or questions, dont hesitate to get in touch.</p>
            </div>
            <br>
            <br>
            <div class="col-12">
      ...
      // ------------------------------------PAYMENT REMIDER--------------------------------------------------------
        if($collection_status == "DUE"){
          // Send email during 10th and 12th day for Phase 1 
        if ($current_day >= date("j", mktime(0, 0, 0, $current_month_num, 10, $current_year)) && $current_day <= date("j", mktime(0, 0, 0, $current_month_num, 12, $current_year)) && $property_phase == "Phase 1") {
          ...
          // Send email during 17 and 19th day for Phase 2 
        elseif ($current_day >= date("j", mktime(0, 0, 0, $current_month_num, 17, $current_year)) && $current_day <= date("j", mktime(0, 0, 0, $current_month_num, 19, $current_year))  && $property_phase == "Phase 2") {
        ...
        // Send email during 23 and 25th day for Phase 3 
        elseif ($current_day >= date("j", mktime(0, 0, 0, $current_month_num, 23, $current_year)) && $current_day <= date("j", mktime(0, 0, 0, $current_month_num, 25, $current_year))  && $property_phase == "Phase 3") {
          // BUG When email sent is failed still updating 
          $sent = "SENT";
          $query3 = "UPDATE collection_list SET email_status = :sent WHERE id = :collection_id_pk AND archive = :ACTIVE";
          $data3 = ["sent" => $sent, "collection_id_pk" => $collection_id_pk, "ACTIVE" => $ACTIVE];
